...
/// Run Phase-1 parameterized factor mining
pub async fn factor_mine_parametric(
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        if !script.exists() {
            return Err("factor_mining.py not found".to_string());
        }
    ...
    flatten_spawn_result(result)
}

/// Run Phase-2 GP factor mining
pub async fn factor_mine_gp(
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        if !script.exists() {
            return Err("gp_factor_mining.py not found".to_string());
        }
        let python = find_python().ok_or("Python not found")?;
...
/// Run lifecycle management on factor registry
...
// ── Helper: run Python script and capture output ────────────────────

fn run_python_script(python: &str, args: &[String]) -> Result<Value, String> {
    let str_args: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
    let output = std::process::Command::new(python)
        .args(&str_args)
        .env("PYTHONIOENCODING", "utf-8")
